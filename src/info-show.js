function TempValue(temp, min, max) {
  ...
  this.minC = `Min: ${min}°C`;
  this.maxC = `Max: ${max}°C`;
  this.tempF = `${Math.round(parseInt(temp, 10) * 1.8 + 32)}°F`;
  this.minF = `Min: ${Math.round(parseInt(min, 10) * 1.8 + 32)}°F`;
  this.maxF = `Max: ${Math.round(parseInt(max, 10) * 1.8 + 32)}°F`;
}
...
const dataContainer = (cityName, temp, icon, min, max, description) => {
  ...
  const temperature = new TempValue(temp, min, max);
